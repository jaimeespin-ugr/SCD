...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 )               // si buffer vacío
         ...
         //MPI_ANY_SOURCE es como un comodín
...
int main( int argc, char *argv[] )
{
   ...
   /*MPI es un estándar que define una biblioteca de funciones 
   para la programación paralela en sistemas de memoria distribuida.
   - Procesos Independientes: Cada programa MPI se ejecuta como un 
   conjunto de procesos independientes, cada uno con su propio espacio 
   de memoria.
   - Comunicación Explícita: La única forma en que los procesos pueden 
   intercambiar información es a través del paso de mensajes 
   (enviar (Send) y recibir (Recv)). No hay variables compartidas.
   */
